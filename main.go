...
func studentLessons() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		// get 1 data students by nis
		// ambil data lessons by student.kelas
		defer ctx.Request.Body.Close()
		dataBody, err := io.ReadAll(ctx.Request.Body)
		if err != nil {
			panic(err)
		...
		lessonList := []MataPelajaran{}

		for _, val := range Lessons {
			if student.Kelas == val.Kelas {
				lessonList = append(lessonList, val)
			}
		}
...
func GetGin() *gin.Engine {
	...
	// get lessons by student nis
	r.POST("/studentLessons", studentLessons())
